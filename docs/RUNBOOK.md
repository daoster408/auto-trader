...
The simplified AI rubric is aggressive and dollar-outcome-first, but evidence-based: missing optional news, fundamentals, or macro context is uncertainty rather than an automatic veto, and late-session candidates are not rejected merely because deterministic exits can hold across sessions. The supervisor starts at most one real-provider candidate research attempt per tick. Cached decisions, deterministic prefilter blocks, and capacity skips do not consume that tick attempt; a completed or failed real-provider attempt ends paid research for the tick so the 90-second supervisor deadline continues to protect monitoring and exit cadence.
...
Use `AI_RESEARCH_XAI_TIMEOUT_SECONDS=60` and `AI_RESEARCH_XAI_REASONING_EFFORT=low` for latency-sensitive live-entry Grok research. Newer reasoning models default to high effort when this is omitted, which can spend thousands of hidden reasoning tokens on a binary entry decision. The default supervisor tick budget is 150 seconds, leaving more than 30 seconds of scanner/context slack beyond the provider deadline. A failed or invalid provider result is fail-closed and is reused only for `AI_PROVIDER_FAILURE_COOLDOWN_SECONDS` (default 300) to prevent immediate duplicate paid attempts; after that cooldown, the symbol may be researched again. Cache-reuse rows stay persisted for audit but are hidden from the default `/ai` latest-decision display. Valid decisions remain cached for the UTC trading day.
